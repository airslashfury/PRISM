...
30-yr NPV factor at 3%      : 19.60  (= (1 - 1.03^-30) / 0.03)

base_voll_per_person = 0.822 × $5 × 33.6 × 19.60 ≈ $2,707 / person

F10b reconciliation (2026-07-10): this used to run its own 4%/yr NPV factor (17.29 →
$2,389/person), while config/confidence.yml's global `discount_rate` — used by the optimizer,
/corridor, and the F4 assumptions panel's sensitivity sweep (`prism.validate.sensitivity.
_npv_factor`) — was 3%/yr. No documented reason favored keeping VOLL on a different rate, so
this now matches the global 3%/yr (see config/assumption_rationale.yml).
...
_DISCOUNT_RATE           = 0.03    # config/confidence.yml's global discount_rate (F10b)
_NPV_FACTOR_30YR         = (1 - (1 + _DISCOUNT_RATE) ** -30) / _DISCOUNT_RATE   # ≈ 19.60

# base_voll_per_person = 0.822 × 5.0 × 33.6 × 19.60 ≈ $2,707 (30-yr)
...
def compute_exposure(engine: Engine, scenario: str = "cat3") -> int:
    ...
    exposure_sql = text("""
        ...
        -- `powered_barrios` DISTINCTs on barrio.entity_id (F10b) — the recursive
        -- `downstream` set is not itself deduplicated (a diamond in the FEEDS graph
        -- can reach the same entity via two path lengths, since the recursive UNION
        -- only dedupes on the full (entity_id, depth) tuple), so without this a
        -- barrio powered through two converging paths was counted twice. This is
        -- the same per-barrio dedup graph.downstream_summary already does in Python.
        LEFT JOIN LATERAL (
            ...
            powered_barrios AS (
                SELECT DISTINCT barrio.entity_id, barrio.geom
                FROM downstream d
