...
test("/lab notebooks: a 5-cell notebook survives reload, re-runs, and permalinks (F13b)", async ({ page, request }) => {
  test.slow(); // multiple cells, each hitting the live prism_ro role / Ask over the network
  const errors: string[] = [];
  page.on("pageerror", (e) => errors.push(e.message));
  ...
  const name = `_e2e_ notebook ${Date.now()}`;
  await page.getByPlaceholder("Notebook name…").fill(name);
  await page.getByRole("button", { name: "New notebook" }).click();
  await expect(page.getByRole("heading", { name, level: 2 })).toBeVisible({ timeout: 15_000 });
  const notebookId = new URL(page.url()).searchParams.get("nb");

  try {
    // Five cells: two query, one sql, one markdown, one ask. Cell-kind badge
    // assertions are scoped to <main> — "Ask PRISM" is also the (hidden,
    // off-canvas on mobile) nav link label, which a bare getByText would match.
    const main = page.getByRole("main");
    await page.getByRole("button", { name: "+ Query" }).click();
    await page.getByRole("button", { name: "+ Query" }).click();
    await page.getByRole("button", { name: "+ SQL" }).click();
    await page.getByRole("button", { name: "+ Markdown" }).click();
    await page.getByRole("button", { name: "+ Ask PRISM" }).click();
    await expect(main.getByText("Query", { exact: true })).toHaveCount(2);
    await expect(main.getByText("SQL", { exact: true })).toHaveCount(1);

    // Regression guard: a keystroke into a fresh, unconfigured Ask cell must
    // NOT auto-submit — the once-on-load auto-run is driven by the persisted
    // spec, not live edit state (a prior fix briefly re-armed it on every
    // canRun flip, firing /ask on the very first typed character).
    const askCalls: string[] = [];
    page.on("request", (r) => {
      if (r.url().endsWith("/api/ask")) askCalls.push(r.url());
    });
    await page.getByPlaceholder("Ask a question about the model…").fill("S");
    await page.waitForTimeout(2_000);
    expect(askCalls).toEqual([]);

    // Pick a NON-default curated query on the first query cell (its default
    // on creation is already the first spec, "Top substations by composite
    // score" — selecting the same one again wouldn't prove a changed spec
    // survives a save, only that the pre-existing default renders) and assert
    // a column unique to that query's result to prove the switch actually ran.
    await page.getByRole("combobox").first().click();
    await page.getByRole("option", { name: "Highest-consequence substations" }).click();
    await page.getByRole("button", { name: "Save & run" }).first().click();
    await expect(main.getByText("population_affected")).toBeVisible({ timeout: 30_000 });

    const urlBeforeReload = page.url();
    expect(urlBeforeReload).toMatch(/[?&]nb=\d+/);

    await page.reload({ waitUntil: "domcontentloaded" });
    await expect(page.getByRole("heading", { name, level: 2 })).toBeVisible({ timeout: 15_000 });
    expect(page.url()).toBe(urlBeforeReload);
    // The saved spec survived (combobox still shows the non-default choice)
    // and the query cell re-runs automatically on load without a manual click.
    await expect(page.getByRole("combobox").first()).toHaveText("Highest-consequence substations");
    await expect(main.getByText("population_affected")).toBeVisible({ timeout: 30_000 });
    await expect(main.getByText("Query", { exact: true })).toHaveCount(2);
    await expect(main.getByText("SQL", { exact: true })).toHaveCount(1);
    await expect(main.getByText("Markdown", { exact: true })).toHaveCount(1);
    await expect(main.getByText("Ask PRISM", { exact: true }).first()).toBeVisible();

    expect(errors, `uncaught page errors on /lab: ${errors.join("; ")}`).toEqual([]);
  } finally {
    // Don't leave e2e litter for the (unrelated) Python fixture to sweep up.
    if (notebookId) await request.delete(`/api/lab/notebooks/${notebookId}`);
  }
});
